#ifndef UNIVERSAL_NOISE_INCLUDED
#define UNIVERSAL_NOISE_INCLUDED
...
half ComputePerlinFBM(half2 uv)
{
    half f = 0;
    uv *= 8.0;
    half2x2 m = half2x2( 1.6,  1.2, -1.2,  1.6 );
    f   =  0.5000 * noise_Perlin( uv ); uv = mul(m, uv);
    f   += 0.2500 * noise_Perlin( uv ); uv = mul(m, uv);
    f   += 0.1250 * noise_Perlin( uv ); uv = mul(m, uv);
    f   += 0.0625 * noise_Perlin( uv ); uv = mul(m, uv);
    f   =  0.5 + 0.5 * f;
    f   *= smoothstep( 0.0, 0.005, abs(uv.x - 0.6) );
    return f;
}

half ComputeSimpleXFBM(half2 uv)
{
    half f = 0;
    uv        *= 8.0;
    half2x2 m   = half2x2( 1.6,  1.2, -1.2,  1.6 );
...
half ComputeValueFBM(half2 uv)
{
    half f = 0;
    uv *= 8.0;
    half2x2 m = half2x2( 1.6,  1.2, -1.2,  1.6 );
    f   =  0.5000 * noise_value( uv ); uv = mul(m, uv );
    f   += 0.2500 * noise_value( uv ); uv = mul(m, uv );
    f   += 0.1250 * noise_value( uv ); uv = mul(m, uv );
    f   += 0.0625 * noise_value( uv ); uv = mul(m, uv );
    f   =  0.5 + 0.5 * f;
    f   *= smoothstep( 0.0, 0.005, abs(uv.x - 0.6) );	
    return f;
}

half ComputeVoronoise(half2 uv)
{
    half  f = 0;
    half2 p = 0.5 - 0.5 * cos(5 * half2(1.0,0.5) );

    p = p * p * (3.0 - 2.0 * p);
    p = p * p * (3.0 - 2.0 * p);
    p = p * p * (3.0 - 2.0 * p);
                
    f = voronoise( 24.0 * uv, p.x, p.y );
    return f;
}
